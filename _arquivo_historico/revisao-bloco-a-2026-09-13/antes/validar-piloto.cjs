...
if (atividades.length !== 6) {
  erros.push(`Esperava-se exatamente 6 atividades didáticas no piloto, mas foram encontradas ${atividades.length}.`);
}
...
atividades.forEach((ativ, idx) => {
  ...
  if (!Array.isArray(ativ.fontes) || ativ.fontes.length === 0) {
    ...
    ativ.fontes.forEach(f => {
      ...
      if (!f.url || !f.url.startsWith('https://drive.google.com')) {
