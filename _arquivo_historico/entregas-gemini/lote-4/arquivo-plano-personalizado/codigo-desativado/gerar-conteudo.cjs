...
const exNotasPedagogicas = {
  ex1: {
    diretrizes: 'Condução melódica cantabile nas cordas agudas (1ª e 2ª cordas). Toque alternado i–m com clareza tímbrica.',
    amortecimento: 'Término no pulso X indica o início exato do pulso X. No compasso 8, resolução em C4 no pulso 1 dura 3 pulsos; pulso 4 é pausa com amortecimento intencional.',
    voicings: 'Melodia na 1ª e 2ª cordas sem acompanhamento simultâneo (monofonia expressiva).',
    entregaU2: null
  },
  ex2: {
    diretrizes: 'Integração de duas vozes independentes: melodia cantabile no topo e linha de baixo no polegar (p).',
    amortecimento: 'Encerramento conjunto no c.8: melodia e baixo soam por 3 pulsos e entram em pausa simultânea no pulso 4.0. O som do baixo é sustentado pela ressonância livre da corda solta; o polegar se afasta após o ataque (não apoia na corda para sustentar).',
    voicings: 'Baixos em Dó (5/3), Fá (4/3) e Sol (6/3) contra melodia em 1ª e 2ª cordas.',
    entregaU2: null
  },
  ex3: {
    diretrizes: 'Arranjo completo em 3 camadas: melodia cantabile (m/a), preenchimento harmônico interno (i) e baixo sustentado (p). Digitação recomendada no compasso 8: melodia com m, voz interna com i.',
    amortecimento: 'No c.8, pausa conjunta de todas as 3 vozes no pulso 4.0. Amortecimento ativo por alívio suave de pressão na mão esquerda sem retirar o contato com a corda.',
    voicings: 'Acordes de C, F e G com voz interna na 3ª corda (G solta).',
    entregaU2: 'A entrega oficial da Unidade 2 exige a execução integral dos 16 compassos da peça (forma de 8 compassos com repetição completa AABB, totalizando entre 69 e 77 segundos a 50–56 BPM), com confirmação explícita na avaliação para liberação de avanço.'
  },
  ex4: {
    diretrizes: 'Tríades e inversões em 3 cordas com melodia superior (4 compassos derivados da fonte JSON).',
    amortecimento: 'Cada acorde soa no pulso 1 e dura 4 pulsos, com encerramento/rearticulação na barra de compasso seguinte.',
    voicings: 'Voicings econômicos nas cordas 1, 2 e 3: c.1 C fundamental (G-E-C); c.2 C/E 1ª inversão (E no baixo); c.3 G/B 1ª inversão (B no baixo); c.4 C resolução.',
    entregaU2: null
  },
  ex5: {
    diretrizes: 'Condução linear de baixos por notas comuns (4 compassos: C -> G/B -> Am -> C/G).',
    amortecimento: 'Cada nota entra no pulso 1, dura 4 pulsos e é encerrada/rearticulada na barra de compasso. Não presumir ligadura entre compassos.',
    voicings: 'Notas comuns C4 (2ª corda, casa 1) e E4 (1ª corda solta) permanecem soando nos compassos 1 a 4 enquanto o baixo desce por graus conjuntos (C -> B -> A -> G).',
    entregaU2: null
  },
  ex6: {
    diretrizes: 'Estudo polifônico com duas inversões justificadas (8 compassos). Integração de melodia, condução linear e inversões harmônicas funcionais.',
    amortecimento: 'Amortecimento ativo deliberado nas quebras de frase e pausas cadenciais.',
    voicings: 'Inversões de 1ª inversão (C/E no c.2 e G/B no c.6). A versão simplificada ex6-s (4 compassos) é estritamente preparação parcial e NÃO substitui a entrega final de 8 compassos para avanço da Unidade 3.',
    entregaU2: null
  }
};
...
const exercicios = [];
const exDefs = [
  { id: 'ex1', sId: 'ex1-s', unidade: 2, numero: 1, titulo: 'Melodia Cantabile de "Primeiro Canto"', bpm: '50–60 BPM' },
  { id: 'ex2', sId: 'ex2-s', unidade: 2, numero: 2, titulo: '"Primeiro Canto" a Duas Vozes (Melodia e Baixo)', bpm: '50–56 BPM' },
  { id: 'ex3', sId: 'ex3-s', unidade: 2, numero: 3, titulo: '"Primeiro Canto" Completo (Arranjo em Três Camadas)', bpm: '50–56 BPM' },
  { id: 'ex4', sId: 'ex4-s', unidade: 3, numero: 1, titulo: 'Tríades e Inversões em Três Cordas com Melodia Superior', bpm: '50–56 BPM' },
  { id: 'ex5', sId: 'ex5-s', unidade: 3, numero: 2, titulo: 'Condução Linear de Baixos por Notas Comuns (Reconstruído)', bpm: '50–56 BPM' },
  { id: 'ex6', sId: 'ex6-s', unidade: 3, numero: 3, titulo: 'Estudo Polifônico com Duas Inversões Justificadas', bpm: '50–56 BPM' }
];
...
for (const def of exDefs) {
  ...
  const totalCompReal = jsonEx && jsonEx.total_compassos ? jsonEx.total_compassos : 8;
  const totalCompSimpl = def.id === 'ex6' ? 4 : (jsonSEx && jsonSEx.total_compassos ? jsonSEx.total_compassos : totalCompReal);
  const notas = exNotasPedagogicas[def.id] || {};
  ...
  exercicios.push({
    ...
    avisoPedagogico: def.id === 'ex3' 
      ? 'A entrega oficial da Unidade 2 exige a execução integral dos 16 compassos (8 compassos com repetição) com confirmação explícita para avanço curricular.'
      : (def.id === 'ex6' ? 'A entrega oficial da Unidade 3 exige esta versão integral de 8 compassos demonstrada em duas sessões distintas.' : null),
    notasPedagogicas: notas
  });
  ...
  exercicios.push({
    ...
    notasPedagogicas: {
      ...notas,
      diretrizes: `${notas.diretrizes || ''} (Versão Simplificada com andamento ou densidade reduzida).`
    }
  });
}
...
5ª     3   =   =   =   |3   =   =   =   `,
  ...
  },
  {
    ...
    tipo: 'principal',
    numero: 4,
    titulo: 'Estudo Preparatório de Oito Compassos da Unidade 1',
    tituloCompleto: 'U1-ESTUDO: Estudo Preparatório de Oito Compassos (Resumo Estrutural da Forma)',
    andamento: '50–60 BPM',
    formula: '4/4',
    totalCompassos: 8,
    ehResumoEstrutural: true,
    tablatura: `[RESUMO ESTRUTURAL DA FORMA — 8 COMPASSOS]
(Fonte: INICIO-4-SEMANAS.md — Este bloco descreve a estrutura formal de montagem, e não uma tablatura completa nota a nota conferida)

c. 1–2: Exercício 1 (melodia cantabile + baixo C em 5/3 sustentado por 4 pulsos)
c. 3–4: Exercício 1 com a voz interna G (3/0) do Exercício 3 no pulso 3
c. 5–6: Exercício 1 executado em dinâmica suave (piano)
c. 7:   Primeiro compasso do Exercício 1
c. 8:   Resolução — Acorde C em 5/3 e C em 2/1 atacados juntos, sustentados por 4 pulsos`,
    descricao: 'Estudo preparatório estruturado a partir das rotinas de INICIO-4-SEMANAS.md. Integra independência melodia/baixo, voz interna suave e contraste de dinâmica.',
    ...
    notasPedagogicas: {
      diretrizes: 'Montagem estrutural em 4 seções contrastantes. Manter o andamento constante mesmo ao fazer o trecho suave nos compassos 5 e 6.',
      amortecimento: 'Corte mecânico deliberado no compasso 8 após 4 pulsos de sustentação.',
      voicings: 'Baixo C em 5/3 sustentado enquanto a melodia caminha na 1ª e 2ª cordas; voz interna G solta (3/0) no pulso 3 dos compassos 3 e 4.'
    }
  }
];
...
const cursoDados = {
  meta: {
    versao: '1.3-lote4-revisado',
    dataGeracao: new Date().toISOString(),
    titulo: 'Curso de Violão — Método Tríade',
    subtitulo: 'Catálogo Original & Meu Plano de Estudo (48 Semanas)',
