...
// Verificação das notas constituintes nos acordes corrigidos e exame da posição superior da melodia
const chordChecks = [
  { ex: 'ex5', c: 1, name: 'C', pitches: ['C3', 'G3', 'E4'], top: 'E4' },
  { ex: 'ex5', c: 2, name: 'G/B', pitches: ['B2', 'G3', 'D4'], top: 'D4' },
  { ex: 'ex5', c: 3, name: 'Am', pitches: ['A2', 'E3', 'C4'], top: 'C4' },
  { ex: 'ex5', c: 4, name: 'C/G', pitches: ['G2', 'C4', 'E4'], top: 'E4' },
  { ex: 'ex6', c: 2, name: 'G/B', pitches: ['B2', 'G3', 'D4'], top: 'D4' },
  { ex: 'ex6', c: 4, name: 'C/G', pitches: ['G2', 'C4', 'E4'], top: 'E4' },
  { ex: 'ex6', c: 7, name: 'C/E', pitches: ['E3', 'G3', 'C4'], top: 'C4' },
];
for(const ch of chordChecks){
  const exObj = d.exercicios.find(x=>x.id===ch.ex);
  if(exObj){
    const cNotes = exObj.eventos.filter(x=>x.compasso===ch.c&&x.tipo!=='pausa');
    const actualPitches = cNotes.map(x=>x.nota_soante);
    for(const p of ch.pitches){
      if(!actualPitches.includes(p)) errs.push(`${ch.ex} c${ch.c} (${ch.name}): nota constituinte ${p} ausente`);
    }
    const melNotes = cNotes.filter(x=>x.voz==='melodia');
    const otherNotes = cNotes.filter(x=>x.voz!=='melodia');
    for(const m of melNotes){
      const pm = pitch(m.nota_soante);
      for(const o of otherNotes){
        if(pitch(o.nota_soante)>pm){
          errs.push(`${ch.ex} c${ch.c}: melodia ${m.nota_soante} abaixo de outra voz ${o.nota_soante}`);
        }
      }
    }
  }
