...
(function () {
  ...
  function carregarSessaoNoCard(sessionId) {
    ...
    s.blocos.forEach(b => {
      ...
      const partes = (window.CursoRegras && window.CursoRegras.parseLinksSeguros)
        ? window.CursoRegras.parseLinksSeguros(b.atividade)
        : [{ tipo: "texto", valor: b.atividade }];
      ...
      partes.forEach(p => {
        if (p.tipo === "link") {
          ...
          a.target = "_blank";
          a.rel = "noopener noreferrer";
          a.className = "link-externo-seguro";
          a.textContent = p.valor;
          tdAtiv.appendChild(a);
        } else {
          tdAtiv.appendChild(document.createTextNode(p.valor));
        }
      });
  ...
  function renderExerciseDisplay(baseExerciseId) {
    ...
    const notesSec = document.getElementById("exercisePedagogicalNotesSection");
    if (notesSec) {
      notesSec.innerHTML = "";
      if (ex.notasPedagogicas && typeof ex.notasPedagogicas === "object") {
        ...
        h4.textContent = "Diretrizes Pedagógicas & Execução Musical";
        card.appendChild(h4);

        const np = ex.notasPedagogicas;
        function addPedNote(label, val) {
          if (!val) return;
          const p = document.createElement("p");
          p.className = "text-sm";
        ...
        addPedNote("Diretriz Geral", np.diretrizes);
        addPedNote("Técnica de Amortecimento", np.amortecimento);
        addPedNote("Voicings & Acordes", np.voicings);
        addPedNote("Encadeamento de Vozes", np.encadeamento);
        addPedNote("Critério de Entrega U2", np.entregaU2);
        addPedNote("Critério de Entrega U3", np.entregaU3);
        addPedNote("Estrutura da Unidade", np.notaEstrutural);

        notesSec.appendChild(card);
      } else {
  ...
  function setupDiarioView() {
    ...
    function checkU2ConfirmationVisibility() {
      const u2Box = document.getElementById("u2ConfirmationContainer");
      const u2Check = document.getElementById("rubricConfirmU2Full");
      if (u2Box) {
        if (rubricUnit.value === "u2" && rubricVer.value === "ex3") {
          u2Box.style.display = "block";
        } else {
          u2Box.style.display = "none";
          if (u2Check) u2Check.checked = false;
        }
      }
    }
    ...
    function updateRubricVersions() {
      ...
      if (uId === "u1") {
        ...
        rubricVer.add(new Option("Ex. 3: 'Primeiro Canto' Completo (16 compassos: 8 c/ repetição)", "ex3"));
    ...
    if (rubricVer) {
      rubricVer.addEventListener("change", checkU2ConfirmationVisibility);
    }
    ...
    if (formRubric) {
      formRubric.addEventListener("submit", (e) => {
        ...
        const u2Check = document.getElementById("rubricConfirmU2Full");
        const isU2Official = (rubricUnit.value === "u2" && rubricVer.value === "ex3");
        const execIntegral = isU2Official ? (u2Check ? u2Check.checked : false) : true;
        ...
        const novaAvaliacao = {
          ...
          execucaoIntegralConfirmada: execIntegral,
  ...
  function checkUnitAdvancement(unidadeId) {
    if (window.CursoRegras && typeof window.CursoRegras.avaliarAvanco === "function") {
      return window.CursoRegras.avaliarAvanco(appState.progresso.avaliacoes, unidadeId);
    }
    ...
    const tomadasCompletasSucesso = evals.filter(e => {
      ...
      const confirmU2 = (unidadeId !== "u2" || e.execucaoIntegralConfirmada === true);
      return atingiuNotas && versaoValida && confirmU2;
    });
    ...
    return {
      ...
      pendenteExecucaoIntegralU2: false
    };
  }
  ...
  function renderAdvancementStatus() {
    ...
    activeUnits.forEach(uId => {
      ...
      const h4 = document.createElement("h4");
      h4.textContent = u ? `Unidade ${u.numero}: ${u.titulo}` : uId;
      ...
      } else if (res.datasDistintas && res.datasDistintas.length === 1) {
      ...
      function buildStatusParagraph(strongTitle, textContent, codeSnippet, suffixText) {
        const strong = document.createElement("strong");
        strong.textContent = strongTitle + " ";
        pParecer.appendChild(strong);
        if (textContent) {
          pParecer.appendChild(document.createTextNode(textContent + " "));
        }
        if (codeSnippet) {
          const code = document.createElement("code");
          code.textContent = codeSnippet;
          pParecer.appendChild(code);
          pParecer.appendChild(document.createTextNode(" "));
        }
        if (suffixText) {
          pParecer.appendChild(document.createTextNode(suffixText));
        }
      }

      if (res.aprovado) {
        const datasTxt = res.datasDistintas.join(" e ");
        buildStatusParagraph(
          "Status Oficial:",
          "Parabéns! Você demonstrou estabilidade na entrega oficial (versão",
          res.requiredVersion,
          `) com nota mínima 2 em todas as 4 dimensões em duas datas distintas (${datasTxt}). O avanço pedagógico está plenamente liberado.`
        );
      } else if (res.pendenteExecucaoIntegralU2) {
        buildStatusParagraph(
          "Atenção da Rubrica:",
          "Você atingiu notas ≥ 2, mas a entrega final da Unidade 2 exige confirmação explícita da execução integral de 16 compassos (forma AABB de 8 compassos com repetição). Marque a confirmação na avaliação para certificar o avanço.",
          null,
          null
        );
      } else if (res.mesmaDataApenas) {
        const dataUnica = res.datasDistintas[0] || "";
        buildStatusParagraph(
          "Atenção da Rubrica:",
          `Você possui ${res.totalTomadasSucesso} tomadas com notas ≥ 2 registradas na data`,
          dataUnica,
          ". Pela regra oficial do Codex, duas tomadas no mesmo dia não bastam para certificar avanço. É obrigatório repetir a gravação em outro dia para comprovar permanência motora."
        );
      } else if (res.datasDistintas && res.datasDistintas.length === 1) {
        const dataUnica = res.datasDistintas[0] || "";
        buildStatusParagraph(
          "Progresso:",
          "1 tomada oficial bem-sucedida registrada em",
          dataUnica,
          ". Realize uma segunda tomada em data posterior para atingir o critério de avanço."
        );
      } else if (res.temParcialApenas) {
        buildStatusParagraph(
          "Aviso de Versão:",
          "Há registros na versão simplificada",
          "ex6-s",
          "(4 compassos). Esta versão é uma preparação parcial de estudo e não atende à entrega final da Unidade 3, que exige os 8 compassos integrais de ex6."
        );
      } else {
  ...
  function renderHistory() {
    ...
    itensFiltrados.forEach(entry => {
      ...
      if (entry.tipo === "pratica") {
        ...
        let scoreTxt = `Pontuação: Continuidade = ${sc.continuidade}/3 | Som = ${sc.clareza}/3 | Equilíbrio = ${sc.equilibrio}/3 | Autonomia = ${sc.autonomia}/3`;
        if (a.unidadeId === "u2" && a.versao === "ex3") {
          scoreTxt += a.execucaoIntegralConfirmada ? " | Execução 16c: Confirmada ✔" : " | Execução 16c: Não confirmada ❌";
        }
        pScores.textContent = scoreTxt;
  ...
  function setupBackupHandlers() {
      ...
      fileInput.addEventListener("change", (e) => {
        ...
        reader.onload = (event) => {
          try {
            ...
            if (!window.CursoRegras || typeof window.CursoRegras.validarBackup !== "function") {
              throw new Error("Módulo CursoRegras não carregado.");
            }

            const validacao = window.CursoRegras.validarBackup(parsed);
            if (!validacao.valido) {
              throw new Error("Erros de validação do backup:\n• " + validacao.erros.join("\n• "));
            }

            dadosImportadosTemp = validacao.dados;
            previewBox.style.display = "block";
            const sessCount = dadosImportadosTemp.progresso.sessoesPraticadas.length;
            const evalCount = dadosImportadosTemp.progresso.avaliacoes.length;
            const assistCount = dadosImportadosTemp.catalogoAssistidos.length;
            const pratCount = dadosImportadosTemp.catalogoPraticados.length;
            previewText.textContent = `Backup válido (versão ${validacao.versao_backup})! Contém ${sessCount} sessões, ${evalCount} avaliações, ${assistCount} aulas assistidas e ${pratCount} praticadas. Escolha como deseja aplicar:`;
          } catch (err) {
            alert(`Erro ao validar backup: ${err.message}`);
    ...
    if (btnMerge) {
      btnMerge.addEventListener("click", () => {
        ...
        if (!window.CursoRegras || typeof window.CursoRegras.mesclarEstado !== "function") {
          alert("Módulo CursoRegras indisponível para mesclagem.");
          return;
        }
        ...
        const resMerge = window.CursoRegras.mesclarEstado(estadoAtual, dadosImportadosTemp);

        appState.progresso = resMerge.novoEstado.progresso;
        appState.catalogoAssistidos = new Set(resMerge.novoEstado.catalogoAssistidos);
        appState.catalogoPraticados = new Set(resMerge.novoEstado.catalogoPraticados);

        salvarProgresso();
        salvarCatalogoTracking();
        alert(`✔ Dados mesclados com sucesso!\n• Novas sessões: ${resMerge.sessoesAdicionadas}\n• Novas avaliações: ${resMerge.avaliacoesAdicionadas}\n• Aulas assistidas adicionadas: ${resMerge.catalogoAssistidosAdicionados}\n• Aulas praticadas adicionadas: ${resMerge.catalogoPraticadosAdicionados}`);
        previewBox.style.display = "none";
        fileInput.value = "";
        dadosImportadosTemp = null;
        updateDashboardStats();
    ...
    if (btnReplace) {
      btnReplace.addEventListener("click", () => {
        ...
        if (!window.CursoRegras || typeof window.CursoRegras.substituirEstado !== "function") {
          alert("Módulo CursoRegras indisponível para substituição.");
          return;
        }

        const novoEstado = window.CursoRegras.substituirEstado(dadosImportadosTemp);

        appState.progresso = novoEstado.progresso;
        appState.catalogoAssistidos = new Set(novoEstado.catalogoAssistidos);
        appState.catalogoPraticados = new Set(novoEstado.catalogoPraticados);
        if (novoEstado.progresso.ultimaSessaoId) {
          appState.currentSessionId = novoEstado.progresso.ultimaSessaoId;
        }
